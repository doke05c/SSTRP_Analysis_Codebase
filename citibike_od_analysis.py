...
column_dict = {
    ...
    "start_cb" : "column13",
    "end_cb" : "column14",
    "origin_geom" : "column15",
    "destination_geom" : "column16"
}   
...
duck_citi_od_connect = duckdb.connect(database="citi_od.duckdb") 

#ENABLE SPATIAL ELEMENT DUCKDB
duck_citi_od_connect.execute("INSTALL spatial;")
duck_citi_od_connect.execute("LOAD spatial;")

#MAKE NAME FOR SQL/DUCKDB TABLE (makes port from previous analyses more convenient)
table_name = "CITI_OD_TABLE"

#CHECK IF PARQUET HAS ALREADY BEEN CREATED, IF NOT, MAKE IT, AND THE TABLE:
parquet_path = Path("../../citibike/2024-citibike-tripdata-full.parquet")
csv_path = Path("../../citibike/2024-citibike-tripdata-full.csv")

if (not (parquet_path.exists())):

    #CONVERT CSV TO PARQUET, FIX COMMA TYPE ISSUE IN START_Id
...
flow_path = Path("citi_flow_matrix.parquet")
if not flow_path.exists():
    ...
    #EXPORT TO CSV
    duck_citi_od_connect.execute("""
        COPY (
            SELECT *
            FROM destination_nearest
        )
        TO 'test_dock_geom.csv'
        (FORMAT CSV, HEADER);
    """)
    ...
    #SAVE
    import pandas as pd
    pd.DataFrame(flow_matrix, columns=['origin_region', 'destination_region', 'rides']).to_parquet(flow_path)
...
df_long = pd.DataFrame(
    ...
    columns=["origin", "destination", "annual_rides"]
)
...
# EXPORT TO CSV
df_matrix.to_csv("2024_citi_flow_matrix_2d.csv")

#CLOSE DATABASE
duck_citi_od_connect.close()